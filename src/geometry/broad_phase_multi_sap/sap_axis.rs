...
impl SAPAxis {
    ...
    pub fn batch_insert(
        ...
        for proxy_id in new_proxies {
            ...
            //if proxy.aabb.mins[dim] > self.max_bound || proxy.aabb.maxs[dim] < self.min_bound {
                println!("self={:?}, dim={}", self, dim);
                println!("proxy.aabb.mins[dim] <= self.max_bound; {} <= {}", proxy.aabb.mins[dim], self.max_bound);
                println!("proxy.aabb.maxs[dim] <= self.min_bound; {} <= {}", proxy.aabb.maxs[dim], self.min_bound);
            //}
